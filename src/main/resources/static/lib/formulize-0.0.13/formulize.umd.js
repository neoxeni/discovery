    ...
    ***************************************************************************** */
    ...
    var StringHelper = /** @class */ (function () {
        ...
        StringHelper.format = function (value) {
            ...
            if (args)
                args.forEach(function (match, index) {
                    var matchValue = match
                    if(typeof match === 'object'){
                        if(match['label'] !== undefined){
                            matchValue = match['label']
                        }else if(match['text'] !== undefined){
                            matchValue = match['text']
                        }else if(match['value'] !== undefined){
                            matchValue = match['value']
                        }
                    }

                    return targetValue = StringHelper.replaceArg(index, targetValue, matchValue);
                });
            return targetValue;
        };
    ...
    var UIElementHelper = /** @class */ (function () {
        ...
        UIElementHelper.getOperatorElement = function (id, value) {
            ...
            return $("<div class=\"" + id + "-item " + id + "-operator"+className+"\">" + (value || '').toLowerCase() + "</div>")[0];
        };
    ...
    var UIDom = /** @class */ (function () {
        ...
        UIDom.prototype.initializeDOM = function () {
            ...
            this.statusBox.insertAfter(this.container);
        };
        ...
        UIDom.prototype.updateStatus = function (valid) {
            ...
            var statusText = valid ? this.options.text.pass : this.options.text.error;
            var statusBaseClasses = ['good', 'error'];
            var statusClasses = valid ? statusBaseClasses : statusBaseClasses.reverse();
            this.statusBox.text(statusText).addClass(this.options.id + "-alert-" + statusClasses[0])
    ...
    var UIManager = /** @class */ (function (_super) {
        ...
        UIManager.prototype.setExpression = function (data) {
            var builder = new Builder(new TreeBuilder());
            return builder.doBuild(data)
        };
        ...
        UIManager.prototype.validate = function (extractor) {
            ...
            if(isValid){
                this.options.text.error = 'error';
            }else{
                this.options.text.error = data;
            }
